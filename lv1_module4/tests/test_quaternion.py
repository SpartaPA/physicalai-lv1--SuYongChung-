...
def test_slerp_endpoints(q_pair):
    ...
    # t=0 → q0과 같은 회전
    q_t0 = slerp(q0, q1, 0.0)
    assert np.allclose(quaternion_to_matrix(q_t0), quaternion_to_matrix(q0))

    # t=1 → q1과 같은 회전
    q_t1 = slerp(q0, q1, 1.0)
    assert np.allclose(quaternion_to_matrix(q_t1), quaternion_to_matrix(q1))
