...
def test_camera_to_base_matches_chain(pipeline, rng):
    ...
    P = np.array([0.05, -0.02, 0.60]) + rng.normal(0.0, 0.1, (50, 3))

    chain = default_chain()
    T_base_camera = chain.get("base", "link") @ chain.get("link", "camera")

    got = pipeline.camera_to_base(P)

    # 모듈 ③ 좌표 체인 결과와 같아야 한다
    assert np.allclose(got, chain.transform("base", "camera", P))
    # 행렬 곱을 직접 적용한 결과와도 같아야 한다
    assert np.allclose(got, transform_points(T_base_camera, P))
    # 형태 보존
...
def test_roundtrip_restores_points(pipeline, rng):
    ...
    # (N,3) 점군: base 로 갔다가 camera 로 되돌리면 원래대로
    P = rng.normal(0.0, 0.2, (64, 3))
    back = pipeline.base_to_camera(pipeline.camera_to_base(P))
    assert np.allclose(back, P)

    # (3,) 단일 점도 형태를 유지하며 왕복 성립
    p = rng.normal(0.0, 0.2, 3)
    back_one = pipeline.base_to_camera(pipeline.camera_to_base(p))
    assert back_one.shape == (3,)
    assert np.allclose(back_one, p)
