...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        if joint_axis not in _ROT:
            raise ValueError(f"joint_axis 는 'x'|'y'|'z' 중 하나여야 합니다. 받은 값={joint_axis!r}")

        self._T_base_link0 = T_base_link   # 관절 각도 0 일 때의 기준값
    ...
    def object_pose_in_base(self, T_camera_object) -> np.ndarray:
        ...
        if T_camera_object.shape != (4, 4):
            raise ValueError(f"T_camera_object 는 4x4 여야 합니다. 받은 shape={T_camera_object.shape}")
